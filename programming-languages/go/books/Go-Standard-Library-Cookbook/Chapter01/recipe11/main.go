...
func main() {
	// The file is openes as a log file to write into
	...
	// This is blocking read from sigChan where the Notify function send the signal
	<-sigChan 

	// After the signal is received all the code behind the read from channel could be considered as a cleanup
	// CLEANUP SECTION
	close(closeChan)
	releaseAllResources()
