...
func main() {
	// Create the cannel where the received singal would be sent
	// The Notify will not block then the signal is sent and the channel is not ready
	...
	go func() {
		...
		case syscall.SIGHUP:
			...
			exitChan <- 0
		case syscall.SIGINT:
			fmt.Println("The process has ben interrupted by CTRL+C")
			exitChan <- 0
